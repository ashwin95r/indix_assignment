...
class barter {
  ...
  def main(inputList: List[String]): String  = {
    ...
    for( i <- 0 to QuestionList.length-1) {
      ...
      if(final_frac.num > 0)
       /* println*/finalStr = finalStr + (final_frac.den + " " + mapOfIndexToItems(from) + " = " + final_frac.num + " " + mapOfIndexToItems(to)).toString
      else
       /* println*/finalStr = finalStr + ("? " + mapOfIndexToItems(from) + " = ? " + mapOfIndexToItems(to)).toString
    }
